...
const createUsersTable = async () => {
  ...
  CREATE TABLE IF NOT EXISTS users (
    ...
    shoes_uploaded_count int DEFAULT 0,
...
const seedShoesTable = async (submittingUserId) => {
	const isTableEmpty = await isShoesTableEmpty();
	if (isTableEmpty) {
		await createShoesTable();
		shoesData.forEach(async (shoe) => {
			const insertQuery = {
				text: `INSERT INTO shoes ( name, brand, color, description, material, technologies, sizing_recommendations, img_URL, submitted_by) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
			};
			...
			try {
				...
				// 	text: `UPDATE users SET shoes_uploaded_count = shoes_uploaded_count + 1 WHERE id = $1`,
				// };
				// await pool.query(updateUserQuery, [submittingUserId]);
			} catch (error) {
...
// const submittingUserId = "exampleUser";
seedShoesTable();
createUsersTable();
